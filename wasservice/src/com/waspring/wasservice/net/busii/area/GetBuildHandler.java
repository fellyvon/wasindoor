...
@Requestable(serverName = "GET_BUILDING_REQ")
public class GetBuildHandler implements IHandler {
	...
	public Response handle(JsonElement data) throws Exception {
		...
		GetBuildRepMessage rep = new GetBuildRepMessage();
		...
		while (rs.next()) {
			...
			rep.BUILDING_DESC = rs.getString("BUILDING_DESC");
			rep.BUILDING_NAME = rs.getString("BUILDING_NAME");
			rep.FLOORS = rs.getString("FLOORS");
			rep.JD = rs.getString("JD");
			rep.WD = rs.getString("WD");
		...
		if (index > 0) {
			...
			map.put("RTN_FLAG", "x");
			map.put("RTN_MSG", "x");
			rep.FLOORS_LIST = ResultToObject.resultToBase(
					GetFloorRepMessage.class, dao
							.getFloorInfo(model.MESSAGE.AREA_NO,
									model.MESSAGE.BUILD_NO, null),map);
			for (int j = 0; j < rep.FLOORS_LIST.size(); j++) {
				GetFloorRepMessage p = (GetFloorRepMessage) rep.FLOORS_LIST
						.get(j);
				...
				p.DOOR_LIST = ResultToObject.resultToBase(
						GetDoorRepMessage.class, dao.getDoorInfo(rep.AREA_NO,
								rep.BUILD_NO, p.FLOOR_NO, null), map);
